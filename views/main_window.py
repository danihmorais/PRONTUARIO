...
class MainWindow(ctk.CTk):
    def __init__(self, controller):
        ...
        self.views["appointment"] = AppointmentWindow(self.content_container, self.controller)
        self.views["record"] = RecordWindow(self.content_container, self.controller)
        self.views["search_pacient"] = spw.SearchPacientView(self.content_container)
    ...
    def show_view(self, view_name):
        ...
        if view_name == "dashboard":
            ...
            self.lbl_breadcrumb.configure(text=" Consultas")
            self._set_btn_active(self.bt_consultas, True)
        elif view_name == "record":
            self.lbl_breadcrumb.configure(text=" Prontuário")
            self._set_btn_active(self.bt_prontuario, True)
        elif view_name == "search_pacient":
    ...
    def _set_btn_active(self, btn, active):
        ...
        btn.configure(fg_color=fg, text_color=tc, hover_color=hc)
    ...
    def _build_sidebar(self):
        self.sidebar = ctk.CTkFrame(self, width=220, fg_color=self._tm.c("WHITE"), border_color=self._tm.c("GRAY_LIGHT"), border_width=1, corner_radius=0)
        ...
        try:
            with Image.open(logo_path) as img:
                logo_img = ctk.CTkImage(light_image=img.copy(), dark_image=img.copy(), size=(20, 20))
            self.logo_img = logo_img
            lbl_logo = ctk.CTkLabel(self.logo_icon, image=self.logo_img, text="")
    ...
    def _nav_section(self, parent_widget, label):
        lbl = ctk.CTkLabel(parent_widget, text=label, text_color=self._tm.c("GRAY"), font=(self._tm.font, 10, "bold"))
    ...
    def _load_icon(self, icon_path):
        try:
            with Image.open(icon_path) as img:
                return ctk.CTkImage(light_image=img.copy(), dark_image=img.copy(), size=(18, 18))
        except Exception:
            return None
    ...
    def _build_topbar(self):
        self.topbar = ctk.CTkFrame(self, height=56, fg_color=self._tm.c("TOPBAR_BG"), corner_radius=0)
