...
class PacientView(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        self.lbl_subtitle = ctk.CTkLabel(
            header,
            text="Consulte, edite e remova pacientes cadastrados",
        ...
        self.fr_search.grid(row=1, column=0, sticky="ew", pady=(0, 16))
        self.fr_search.grid_columnconfigure(1, weight=1)
        ...
        self.lbl_search.grid(row=0, column=0, padx=(16, 10), pady=14)
        ...
        self.table_cont.grid(row=2, column=0, sticky="nsew")
        self.table_cont.grid_rowconfigure(1, weight=1)
        self.table_cont.grid_columnconfigure(0, weight=1)
        ...
        self.fr_header.grid(row=0, column=0, sticky="ew")
        ...
        try:
            self.scroll._scrollbar.grid_configure(
                padx=(0, 6)
            )
        except Exception:
            pass
        ...
        if self._on_select:
            self.btn_select = ctk.CTkButton(
                ...
                hover_color=self._tm.c("SUCCESS"),
                text_color="white",
                font=(self._tm.font, 13, "bold"),
                corner_radius=8,
    ...
    def _build_header(self):
        for w in self.fr_header.winfo_children():
            w.destroy()
    ...
    def _render_rows(self, rows):
        ...
        for idx, row in enumerate(rows):
            ...
            for val, w in zip(row, self.COL_WIDTHS):
                ...
                lbl.bind(
                    "<Button-1>",
                    lambda _e, f=fr: self._select_row(f)
                )
    ...
    def _excluir(self):
        ...
        try:
            ...
            cur.execute(
                "DELETE FROM prontuarios WHERE id_paciente = ?",
                (pid,)
            )

            cur.execute(
                "DELETE FROM consultas WHERE id_paciente = ?",
                (pid,)
            )
    ...
    def _on_theme_change(self, colors):
        try:
            ...
            self.ent_search.configure(
                ...
                border_color=colors["GRAY_LIGHT"],
                text_color=colors["BLACK"],
                placeholder_text_color=colors["GRAY"]
            )
            ...
            self.table_cont.configure(
                fg_color=colors["WHITE"],
                border_color=colors["GRAY_LIGHT"]
            )

            self.fr_header.configure(
...
class EditPacientDialog(ctk.CTkToplevel):
    ...
    def _build(self):
        self.lbl_title = ctk.CTkLabel(
            self,
            text="Editar Dados do Paciente",
            font=(self._tm.font, 18, "bold"),
            text_color=self._tm.c("BLACK"),
        )

        self.lbl_title.pack(anchor="w", padx=24, pady=(18, 12))

        self.cont = ctk.CTkScrollableFrame(
            self,
            fg_color="transparent"
        )
        ...
        try:
            self.cont._scrollbar.grid_configure(
                padx=(0, 6)
            )
        except Exception:
            pass

        self.cont.grid_columnconfigure((0, 1), weight=1)
    ...
    def _salvar(self):
        ...
        try:
            ...
            cur.execute(
                f"UPDATE pacientes SET {sets} WHERE id = ?",
                params
            )
