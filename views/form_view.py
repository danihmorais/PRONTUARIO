...
class FormView(ctk.CTkFrame):
    ...
    def _apply_theme(self, colors):
        ...
        try:
            self._tab_pac.configure(fg_color=colors["WHITE"])
            self._tab_fisio.configure(fg_color=colors["WHITE"])
            self._tab_func.configure(fg_color=colors["WHITE"])
        except Exception:
            pass
    ...
    def _build_ui(self):
        ...
        subtitle = ctk.CTkLabel(
            header,
            text="Cadastre pacientes, fisioterapeutas e funcionários",
        ...
        self._tw(self._tab_pac, fg_color="WHITE")
        self._tw(self._tab_fisio, fg_color="WHITE")
        self._tw(self._tab_func, fg_color="WHITE")
    ...
    def _section(self, parent, title):
        ...
        self._tw(line, fg_color="GRAY_LIGHT")
    ...
    def _field(
        ...
        if calendar:
            btn = ctk.CTkButton(
                ...
                command=lambda e=entry: self.calendar_popup.open(e)
            )

            btn.grid(
                row=row * 2 + 3,
                column=col + colspan - 1,
                sticky="e",
                padx=16,
                pady=(0, 12)
            )
    ...
    def _build_paciente(self):
        ...
        geral.pack(fill="x", padx=10, pady=(10, 0))

        self.p_nome = self._field(
            geral,
            "Nome completo",
            0,
            0,
        ...
        end.pack(fill="x", padx=10, pady=(10, 0))
        ...
        contato.pack(fill="x", padx=10, pady=(10, 0))

        self.p_email = self._field(contato, "E-mail", 0, 0, 2)

        self.p_celular = self._field(
            contato,
            "Celular",
            0,
            2,
    ...
    def _salvar_fisioterapeuta(self):
        ...
        try:
            ...
            cur.execute(
                "SELECT id FROM fisioterapeutas WHERE cpf = ?",
                (_only_digits(cpf),)
            )
            ...
            messagebox.showinfo(
                "Sucesso",
                f"Fisioterapeuta '{nome}' cadastrado com sucesso!"
            )
